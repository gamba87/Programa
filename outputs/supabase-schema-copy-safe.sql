...
create trigger trg_receipt_documents_updated_at before update on receipt_documents for each row execute function set_updated_at();
...
as 'begin
  ...
  if tg_op = ''UPDATE'' then
    if old.status <> ''DRAFT'' and current_setting(''app.receipt_document_rpc'', true) <> ''on'' then
      raise exception ''Confirmed or cancelled documents cannot be edited directly.'';
    end if;

    if old.status = ''DRAFT'' and new.status <> ''DRAFT'' and current_setting(''app.receipt_document_rpc'', true) <> ''on'' then
      raise exception ''Document status can only be changed by confirm or cancel operation.'';
    end if;
  end if;
...
create trigger trg_protect_receipt_documents before update or delete on receipt_documents for each row execute function protect_receipt_document_changes();
...
begin
  ...
  if v_status <> ''DRAFT'' then
    raise exception ''Confirmed or cancelled document lines cannot be changed.'';
  end if;
...
create trigger trg_protect_receipt_document_lines before insert or update or delete on receipt_document_lines for each row execute function protect_receipt_document_lines();
...
as 'begin
  if current_setting(''app.receipt_document_rpc'', true) <> ''on'' then
    raise exception ''Stock movements can only be created by document confirm or cancel operation.'';
  end if;
  if tg_op = ''DELETE'' then
    return old;
  end if;
...
create trigger trg_protect_stock_movements before insert or update or delete on stock_movements for each row execute function protect_stock_movements();
